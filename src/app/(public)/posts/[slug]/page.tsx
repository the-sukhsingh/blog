...
export default async function PostDetailPage({ params }: Props) {
  ...
  // Background resolution logic: Article level > Category level > Default
  const resolvedLight =
    post.bgColorLight ?? post.category?.bgColorLight ?? null;
  const resolvedDark = post.bgColorDark ?? post.category?.bgColorDark ?? null;

  return (
    <div className="mx-auto max-w-7xl py-4 md:py-8 px-4 sm:px-0">
      <div className="flex items-start justify-center gap-8 xl:gap-12 relative">
        ...
        <div className="min-w-0 max-w-5xl flex-1">
          ...
          >
            <main>
              ...
              <h1
                id="article-title"
                className="mb-4 font-heading text-3xl font-extrabold leading-tight tracking-tight text-foreground sm:text-4xl md:text-5xl"
              >
                {post.title}
              </h1>
              ...
              {post.excerpt && (
                <p
                  id="article-subtitle"
                  className="mb-6 text-lg text-muted-foreground leading-relaxed font-medium"
                >
                  {post.excerpt}
                </p>
              )}
              ...
              {/* Content */}
              <article
                className="typeset prose max-w-none"
